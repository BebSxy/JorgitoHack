...
import me.ijese.jorgitohack.features.modules.player.XCarry;
...
public class AutoArmor
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoArmor.mc.currentScreen instanceof JorgitoHackGui) && this.elytraBind.getValue().getKey() == Keyboard.getEventKey()) {
    ...
    public void onTick() {
        ...
        if (this.taskList.isEmpty()) {
            ...
            ItemStack chest;
            int slot4;
            if (this.mendingTakeOff.getValue().booleanValue() && InventoryUtil.holdingItem(ItemExpBottle.class) && AutoArmor.mc.gameSettings.keyBindUseItem.isKeyDown() && (this.isSafe() || EntityUtil.isSafe(AutoArmor.mc.player, 1, false, false))) {
                int bootDamage;
                int leggingDamage;
                int chestDamage;
                int helmDamage;
        ...
        if (this.timer.passedMs((int) ((float) this.delay.getValue().intValue() * (this.tps.getValue() != false ? JorgitoHack.serverManager.getTpsFactor() : 1.0f)))) {
            if (!this.taskList.isEmpty()) {
