...
public class BowSpam
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (this.autoSwitch.getValue().booleanValue() && InventoryUtil.findHotbarBlock(ItemBow.class) != -1 && this.ownHealth.getValue().floatValue() <= EntityUtil.getHealth(BowSpam.mc.player) && (!this.onlyWhenSave.getValue().booleanValue() || EntityUtil.isSafe(BowSpam.mc.player))) {
            ...
            if (!(target == null || (crystal = JorgitoHack.moduleManager.getModuleByClass(AutoCrystal.class)).isOn() && InventoryUtil.holdingItem(ItemEndCrystal.class))) {
                ...
                JorgitoHack.rotationManager.lookAtVec3d(xPos, yPos, zPos);
                if (BowSpam.mc.player.getHeldItemMainhand().getItem() instanceof ItemBow) {
                    this.switched = true;
                }
            }
        ...
        if (this.mode.getValue() == Mode.FAST && (this.offhand || BowSpam.mc.player.inventory.getCurrentItem().getItem() instanceof ItemBow) && BowSpam.mc.player.isHandActive()) {
            ...
            float f3 = this.tpsSync.getValue() != false ? JorgitoHack.serverManager.getTpsFactor() : 1.0f;
            if (f >= f2 * f3) {
    ...
    public void onUpdate() {
        ...
        switch (this.mode.getValue()) {
            case AUTORELEASE: {
                if (!this.offhand && !(BowSpam.mc.player.inventory.getCurrentItem().getItem() instanceof ItemBow) || !this.timer.passedMs((int) ((float) this.delay.getValue().intValue() * (this.tpsSync.getValue() != false ? JorgitoHack.serverManager.getTpsFactor() : 1.0f))))
                    break;
    ...
    private EntityPlayer getTarget() {
        ...
        for (EntityPlayer player : BowSpam.mc.world.playerEntities) {
            if (player == null || EntityUtil.isDead(player) || EntityUtil.getHealth(player) > this.health.getValue().floatValue() || player.equals(BowSpam.mc.player) || JorgitoHack.friendManager.isFriend(player) || BowSpam.mc.player.getDistanceSq(player) > MathUtil.square(this.range.getValue().floatValue()) || !BowSpam.mc.player.canEntityBeSeen(player) && !EntityUtil.canEntityFeetBeSeen(player))
                continue;
            if (target == null) {
                target = player;
