...
extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, ((Number)this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float)this.width + 7.4f) * this.partialMultiplier(), this.y + (float)this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? JorgitoHack.colorManager.getColorWithAlpha(JorgitoHack.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : JorgitoHack.colorManager.getColorWithAlpha(JorgitoHack.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        JorgitoHack.textManager.drawStringWithShadow(this.getName() + " " + (Object)ChatFormatting.RED + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf(((Number)this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 4.0f - (float)JorgitoHackGui.getClickGui().getTextOffset(), -1);
    }
