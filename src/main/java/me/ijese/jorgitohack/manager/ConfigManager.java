...
public class ConfigManager implements Util {
    ...
    public String config = "jorgitohack/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        JorgitoHack.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }

    public
    void loadConfig ( String name ) {
        this.loadingConfig = true;
        final List < File > files = Arrays.stream ( Objects.requireNonNull ( new File ( "jorgitohack" ).listFiles ( ) ) ).filter ( File::isDirectory ).collect ( Collectors.toList ( ) );
        this.config = files.contains ( new File ( "jorgitohack/" + name + "/" ) ) ? "jorgitohack/" + name + "/" : "jorgitohack/config/";
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    JorgitoHack.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("jorgitohack").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("jorgitohack/" + name + "/"));
    }

    public
    void saveConfig ( String name ) {
        this.savingConfig = true;
        this.config = "jorgitohack/" + name + "/";
        File path = new File ( this.config );
        if ( ! path.exists ( ) ) {
            path.mkdir ( );
        }
        JorgitoHack.friendManager.saveFriends ( );
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("jorgitohack/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("jorgitohack", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(JorgitoHack.moduleManager.modules);
        this.features.add(JorgitoHack.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        JorgitoHack.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            JorgitoHack.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
