...
public class TextManager
    ...
    private CustomFont customFont = new CustomFont(new Font("Verdana", 0, 19), true, false);
    ...
    public void init(boolean startup) {
        FontMod cFont = JorgitoHack.moduleManager.getModuleByClass(FontMod.class);
        try {
    ...
    float drawString(String text, float x, float y, int color, boolean shadow) {
        if (JorgitoHack.moduleManager.isModuleEnabled(FontMod.class)) {
            if (shadow) {
    ...
    public int getStringWidth(String text) {
        if (JorgitoHack.moduleManager.isModuleEnabled(FontMod.getInstance().getName())) {
